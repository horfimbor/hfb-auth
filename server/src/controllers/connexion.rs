...
pub async fn login(
    ...
    let model = state_repository.get_model(&key).await.map_err(|event_source_error| {
    ...
    let Some(password_hash) = user.password_hash() else {
        return Err(redirect_failed_login(&login, cookies, "Database password corrupted")) ;
    };

    let parsed_hash = PasswordHash::new(&password_hash).map_err(|e| {
        redirect_failed_login(&login, cookies, "Password hashing failed")
    })?;
    ...
    if !login.redirect.is_empty() {
        ...
        dbg!(&redirect);
        return Ok(Redirect::temporary(uri!(controllers::authorization::authorize(redirect))));
    }
